...
exports.timTruyen = exports.truyenTheoLop = exports.TRUYEN_NGHE = void 0;
exports.kiemTruyen = kiemTruyen;
exports.TRUYEN_NGHE = [
...
const truyenTheoLop = (lop) => exports.TRUYEN_NGHE.filter((t) => t.lop === lop);
exports.truyenTheoLop = truyenTheoLop;
const timTruyen = (ma) => exports.TRUYEN_NGHE.find((t) => t.ma === ma);
exports.timTruyen = timTruyen;
/**
...
function kiemTruyen() {
    ...
    for (const t of exports.TRUYEN_NGHE) {
        if (daThay.has(t.ma))
            loi.push(`${t.ma}: trùng mã`);
        daThay.add(t.ma);
        if (t.cau.length < 4)
            loi.push(`${t.ma}: chỉ có ${t.cau.length} câu`);
        if (t.hoi.length < 3)
            loi.push(`${t.ma}: chỉ có ${t.hoi.length} câu hỏi`);
        for (const c of t.cau)
            if (!c.en.trim() || !c.vi.trim())
                loi.push(`${t.ma}: câu thiếu bản dịch`);
        for (const h of t.hoi) {
            if (h.dapAn.length < 3)
                loi.push(`${t.ma}: "${h.hoi}" chỉ có ${h.dapAn.length} đáp án`);
            if (new Set(h.dapAn.map((x) => x.toLowerCase())).size !== h.dapAn.length)
                loi.push(`${t.ma}: "${h.hoi}" có đáp án trùng nhau`);
            if (!h.hoi.trim().endsWith('?'))
                loi.push(`${t.ma}: "${h.hoi}" không phải câu hỏi`);
            if (!h.hoiVi.trim())
                loi.push(`${t.ma}: "${h.hoi}" thiếu bản dịch`);
        }
    }
    return loi;
}
