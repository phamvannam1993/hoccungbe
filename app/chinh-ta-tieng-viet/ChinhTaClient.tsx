...
import { CHINH_TA_TOPICS } from '../lib/chinhTa';
...
export default function ChinhTaClient() {
  const [ti, setTi] = useState(0);
  const [answers, setAnswers] = useState<Record<number, number>>({});
  const topic = CHINH_TA_TOPICS[ti];
  ...
  const choose = (qi: number, oi: number) => {
    ...
    if (oi === q.correct) { playCorrect(); confetti('small'); speakText(q.word); }
    else { playWrong(); }
  };
  ...
  return (
    <div className="mt-6">
      <div className="flex flex-wrap gap-2">
        {CHINH_TA_TOPICS.map((t, i) => (
          <button
            key={t.slug}
            type="button"
            onClick={() => switchTopic(i)}
            className={`rounded-full border-2 px-4 py-2 text-sm font-black transition ${
              i === ti ? 'border-transparent bg-gradient-to-r from-rose-500 to-orange-400 text-white shadow' : 'border-slate-200 bg-white text-slate-600 hover:border-rose-300'
            }`}
          >
            {t.emoji} {t.title}
          </button>
      ...
      <div className="mt-4 rounded-3xl border-2 border-rose-100 bg-rose-50/60 p-5">
      ...
      <div className="mt-4 flex items-center justify-between rounded-2xl bg-white px-4 py-2 shadow-sm ring-1 ring-slate-100">
        <span className="text-sm font-black text-slate-700 kid-display">✍️ Chọn đáp án viết đúng chính tả</span>
        <span className="rounded-full bg-rose-100 px-3 py-1 text-sm font-black text-rose-700">Đúng {score}/{topic.questions.length}</span>
      </div>
            ...
            <li key={qi} className="rounded-3xl border-2 border-slate-100 bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {q.options.map((op, oi) => {
                  ...
                  let cls = 'border-slate-200 bg-white text-slate-700 hover:border-rose-400 hover:bg-rose-50';
                  if (done) {
              ...
              {done && (
                <div className={`mt-3 flex items-start gap-2 rounded-2xl px-3 py-2 text-sm ${right ? 'bg-emerald-50 text-emerald-800' : 'bg-orange-50 text-orange-800'}`}>
                  ...
                  <span className="flex-1">Viết đúng: <b>{q.word}</b>. {q.explain}</span>
                  <button type="button" onClick={() => { unlockAudio(); speakText(q.word); }} className="shrink-0 text-rose-500" aria-label="Nghe từ đúng">🔊</button>
                </div>
              )}
            </li>
          );
      ...
      {answered === topic.questions.length && (
        <div className="mt-4 rounded-3xl border-2 border-emerald-200 bg-emerald-50 p-5 text-center">
          ...
          <button type="button" onClick={() => { setAnswers({}); stopSpeaking(); }} className="mt-3 rounded-full bg-gradient-to-r from-rose-500 to-orange-400 px-6 py-2 text-sm font-black text-white shadow">
            🔁 Làm lại
          </button>
        </div>
      )}
