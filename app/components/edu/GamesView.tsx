...
export default function GamesView() {
  ...
  const visibleGames = gamesData.filter((g) => {
  ...
  const readyCount = gamesData.filter((g) => g.status === 'ready').length;
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-6 pb-4">
        ...
        <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="mt-1 text-white/80 text-sm">{gamesData.length} trò chơi · {readyCount} sẵn sàng · {gameGroups.length} nhóm học tập</p>
          </div>
          {/* Search */}
          ...
          {gameGroups.map((g) => (
            <button
              ...
              className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold transition flex items-center gap-1.5
                ${activeGroup === g.key ? 'bg-white text-[#c0392b]' : 'bg-white/20 text-white hover:bg-white/30'}`}
            >
              <span>{g.emoji}</span>
              {g.title}
            </button>
        ...
        {visibleGames.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {visibleGames.map((game) => {
              const group = gameGroups.find((g) => g.key === game.groupKey);
              ...
              return (
                ...
                >
                  {/* Color bar */}
                  <div className={`h-1.5 bg-gradient-to-r ${group?.colorClass ?? 'from-sky-400 to-violet-400'}`} />
                  ...
                  <div className="flex flex-col flex-1 p-4">
                    ...
                    <p className="mt-1.5 text-xs leading-relaxed text-slate-500 line-clamp-2">{game.shortDescription}</p>

                    {/* Tags */}
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-slate-600">{game.category}</span>
                      <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-slate-600">{game.time}</span>
