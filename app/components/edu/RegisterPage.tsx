    ...
    <section className="mx-auto max-w-6xl px-4 sm:px-6 py-8">
      <div className="grid overflow-hidden rounded-[36px] bg-white shadow-sm ring-1 ring-slate-100 lg:grid-cols-2">
        <div className="bg-gradient-to-br from-emerald-500 via-sky-500 to-violet-500 p-8 text-white lg:p-10">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-white/80">Tạo tài khoản mới</p>
          ...
          <div className="mt-8 space-y-4">
            {['Thiết lập hồ sơ theo độ tuổi của bé', 'Nhận gợi ý trò chơi phù hợp mỗi ngày', 'Theo dõi tiến độ học tập rõ ràng'].map((item) => (
              <div key={item} className="rounded-2xl bg-white/10 px-4 py-3 text-sm font-medium backdrop-blur">
                {item}
              </div>
            ))}
          </div>
        ...
        <div className="p-8 lg:p-10">
          <div className="mx-auto max-w-md">
            <h2 className="text-2xl font-black tracking-tight text-slate-900">Đăng ký tài khoản</h2>
            <p className="mt-2 text-sm leading-7 text-slate-600">
              Tạo tài khoản để lưu tiến độ và cá nhân hóa lộ trình học cho bé.
            </p>

            {error && (
              <div className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-medium text-red-600 ring-1 ring-red-100">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-5">
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-700">Họ và tên phụ huynh</label>
                <input
                  type="text"
                  value={fullName}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full rounded-full bg-gradient-to-r from-emerald-500 via-sky-500 to-violet-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-sky-100 transition duration-300 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-sky-200 disabled:opacity-60"
              >
