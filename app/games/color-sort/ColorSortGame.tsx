...
import {
  colorSortCategories,
  colorSortData,
  type ColorSortQuestion,
} from './data';
import { speakText, stopSpeaking } from '../../components/edu/utils/speech';

function buildQuestionSpeech(question: ColorSortQuestion) {
    return `Bé hãy chọn đồ vật có màu ${question.targetLabel}.`;
  }
  ...
  function buildWrongSpeech(color: string, correctLabel: string) {
    return `Chưa đúng nhé. Đáp án đúng là ${correctLabel}, màu ${color}.`;
  }
...
type PlayQuestion = ColorSortQuestion & {
  shuffledOptions: {
    emoji: string;
    label: string;
    color: string;
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-color-sort-scores';
...
function buildPlayQuestions(source: ColorSortQuestion[], count: number): PlayQuestion[] {
...
export default function ColorSortGame() {
  ...
  const startCategoryGame = (key: CategoryKey) => {
    const sourceQuestions = colorSortData[key].questions;
  ...
  const handleChoose = (option: string) => {
    ...
    const selectedOption = currentQuestion.shuffledOptions.find(
      (item) => item.emoji === option
    );
    const correctOption = currentQuestion.shuffledOptions.find(
      (item) => item.emoji === currentQuestion.correct
    );
    ...
    if (option === currentQuestion.correct) {
      ...
      speakText(buildCorrectSpeech(currentQuestion.targetLabel), {
        lang: 'vi-VN',
        rate: 0.92,
        pitch: 1.12,
      });
      ...
      speakText(
        buildWrongSpeech(
          ...
          correctOption?.label ?? 'đáp án đúng'
        ),
        {
          lang: 'vi-VN',
          rate: 0.9,
  ...
  const handleNext = () => {
    ...
    if (currentIndex === questions.length - 1) {
      ...
      if (selectedCategory) {
        speakText(
          buildFinishSpeech(
            colorSortData[selectedCategory].label,
            score,
            questions.length
          ),
          {
      ...
      <section className="mx-auto max-w-7xl px-6 py-8 lg:px-8 lg:py-12">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé nhìn yêu cầu màu sắc và chọn đúng đồ vật tương ứng. Trò chơi giúp bé
            nhận biết màu nhanh hơn, tăng quan sát và phản xạ khi phân loại.
          </p>

          <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {colorSortCategories.map((category) => (
              <button
                key={category.key}
              ...
              >
                ...
                <p className="mt-2 text-sm leading-7 text-slate-600">
                  {Math.min(QUESTIONS_PER_GAME, category.total)} câu mỗi lượt chơi, nhiều màu sắc quen thuộc.
                </p>
  ...
  if (finished) {
    const categoryInfo = colorSortData[selectedCategory];
  ...
  const colorBadgeMap: Record<string, string> = {
    đỏ: 'bg-rose-50 text-rose-700 ring-rose-100',
    vàng: 'bg-amber-50 text-amber-700 ring-amber-100',
    'xanh lá': 'bg-emerald-50 text-emerald-700 ring-emerald-100',
    'xanh dương': 'bg-sky-50 text-sky-700 ring-sky-100',
    tím: 'bg-violet-50 text-violet-700 ring-violet-100',
    cam: 'bg-orange-50 text-orange-700 ring-orange-100',
    hồng: 'bg-pink-50 text-pink-700 ring-pink-100',
    nâu: 'bg-amber-100 text-amber-900 ring-amber-200',
  };
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé nhìn yêu cầu màu sắc và chọn đúng đồ vật tương ứng. Trò chơi giúp tăng khả năng
            nhận biết màu, quan sát và phản xạ lựa chọn.
          </p>
          ...
          <div className="mt-8 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-slate-50 p-6 text-center ring-1 ring-slate-100">
                <p className="text-sm font-semibold text-slate-500">Màu cần tìm</p>
                <button
                    type="button"
                    onClick={() =>
                        speakText(`Màu ${currentQuestion.targetLabel}`, {
                        lang: 'vi-VN',
                        rate: 0.9,
                        pitch: 1.1,
                        })
                    }
                    className="mt-4 inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm transition duration-300 hover:-translate-y-0.5 hover:border-sky-300 hover:bg-sky-50 hover:text-sky-700"
                    >
                    🎧 Nghe tên màu
                 </button>
                ...
                <div className="mt-4 flex justify-center">
                  <span
                    className={`rounded-full px-6 py-3 text-lg font-black ring-1 ${
                      colorBadgeMap[currentQuestion.targetLabel] ??
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {currentQuestion.shuffledOptions.map((option) => {
                  const isCorrect = option.emoji === currentQuestion.correct;
                  ...
                  return (
                    <button
                      ...
                      onClick={() => handleChoose(option.emoji)}
                      disabled={showResult}
                    ...
                    >
                      ...
                      <div className="mt-2 text-xs text-slate-500">{option.color}</div>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {colorSortData[selectedCategory].icon}
              </div>
              <div>
