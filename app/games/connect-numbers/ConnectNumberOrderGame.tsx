...
import {
  ...
  connectNumberData,
  type ConnectNumberCategory,
  type ConnectNumberLevel,
  type ConnectNumberQuestion,
  type ConnectNumberPoint,
} from './data';

type CategoryKey = keyof typeof connectNumberData;
...
type PlayQuestion = ConnectNumberQuestion;
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-connect-number-order-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-connect-number-order-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-connect-number-order-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-connect-number-order-stickers';
...
const stickers: StickerItem[] = [
  ...
  {
    id: 'one-to-five-master',
    emoji: '1️⃣',
    title: 'Bạn của số nhỏ',
    description: 'Hoàn thành chủ đề từ 1 đến 5.',
  },
  {
    id: 'mixed-master',
    emoji: '✨',
    title: 'Bé nối số siêu tốt',
...
export default function ConnectNumberOrderGame() {
  const [selectedCategory, setSelectedCategory] = useState<CategoryKey | null>(null);
  const [selectedLevel, setSelectedLevel] = useState<ConnectNumberLevel>('easy');
  ...
  const expectedNextNumber = useMemo(() => {
    if (!currentQuestion) return 1;
    return currentQuestion.startNumber + selectedNodes.length;
  }, [currentQuestion, selectedNodes.length]);
  ...
  useEffect(() => {
    ...
    const categoryInfo = connectNumberData[selectedCategory];
    ...
    if (selectedCategory === 'one-to-five') unlockSticker('one-to-five-master');
  ...
  const startCategoryGame = (key: CategoryKey, level: ConnectNumberLevel) => {
  ...
  const handleSelectNumber = async (point: ConnectNumberPoint) => {
    if (!currentQuestion || showResult) return;

    const alreadySelected = selectedNodes.some((item) => item.id === point.id);
    if (alreadySelected) return;

    await playSelectSound();

    const expected = currentQuestion.startNumber + selectedNodes.length;
    ...
    const nextSelected = [...selectedNodes, { id: point.id, value: point.value }];
    setSelectedNodes(nextSelected);

    const completed = point.value === currentQuestion.endNumber;
  ...
  const selectedLines = useMemo(() => {
    if (!currentQuestion || selectedNodes.length < 2) return [];

    const byId = new Map(currentQuestion.points.map((point) => [point.id, point]));

    const lines = [];
    for (let i = 0; i < selectedNodes.length - 1; i += 1) {
      const current = byId.get(selectedNodes[i].id);
      const next = byId.get(selectedNodes[i + 1].id);
      if (!current || !next) continue;

      const p1 = getPointCoordinates(current);
      const p2 = getPointCoordinates(next);
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé bấm các số theo đúng thứ tự tăng dần để hoàn thành đường nối. Trò chơi giúp
                luyện nhận biết số, ghi nhớ thứ tự và tăng phối hợp mắt tay.
              </p>
            </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {connectNumberCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-600">
                    {category.total} màn theo chủ đề này.
                  </p>

                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as ConnectNumberLevel[]).map((level) => (
                      <button
                        key={level}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng nhận biết và nối số theo đúng thứ tự của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {connectNumberData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy bấm lần lượt các số theo đúng thứ tự tăng dần.
          </p>
          ...
          <div className="mt-6 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-4 ring-1 ring-sky-100">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                  <div className="text-sm font-semibold text-slate-600">
                    Hãy bấm từ <span className="font-black text-slate-900">{currentQuestion.startNumber}</span> đến{' '}
                    <span className="font-black text-slate-900">{currentQuestion.endNumber}</span>
                  </div>

                  <div className="rounded-full bg-white px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                    Số tiếp theo: {expectedNextNumber}
                  </div>
                </div>

                <div className="relative h-[360px] overflow-hidden rounded-[28px] bg-white ring-1 ring-slate-200">
                  ...
                  {currentQuestion.points.map((point) => {
                    const isSelected = selectedNodes.some((item) => item.id === point.id);
                    const isNext = point.value === expectedNextNumber && !showResult;
                    const isWrongAfterFinish =
                      showResult && !isWin && point.value === expectedNextNumber;

                    return (
                      <button
                        key={point.id}
                        onClick={() => handleSelectNumber(point)}
                        disabled={showResult}
                        className={`absolute flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full text-lg font-black shadow-sm ring-2 transition ${
                          isSelected
                            ? 'bg-emerald-500 text-white ring-emerald-200'
                            : isNext
                            ? 'bg-violet-500 text-white ring-violet-200'
                            : isWrongAfterFinish
              ...
              <div className="mt-6 rounded-3xl bg-emerald-50 p-4 ring-1 ring-emerald-100">
                ...
                <div className="mt-3 flex min-h-[48px] flex-wrap gap-2">
                  {selectedNodes.length === 0 ? (
                    ...
                    selectedNodes.map((item) => (
                      <div
                        key={item.id}
                        className="rounded-full bg-white px-3 py-2 text-sm font-bold text-slate-800 ring-1 ring-emerald-100"
                      >
                        {item.value}
                      </div>
                    ))
                ...
                <div className="mt-4">
                  <button
                    ...
                    disabled={showResult || selectedNodes.length === 0}
        ...
        <div className="space-y-6">
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {connectNumberData[selectedCategory].icon}
              </div>
              <div>
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="rounded-2xl bg-slate-50 px-4 py-4 text-sm leading-7 text-slate-600 ring-1 ring-slate-100">
                Đã nối: <span className="font-bold text-slate-900">{selectedNodes.length}</span>{' '}
                điểm
              </div>
            </div>
          </div>
