...
import {
  firstLetterCategories,
  firstLetterData,
  type FirstLetterQuestion,
} from './data';
import { speakText, stopSpeaking } from '../../components/edu/utils/speech';

type CategoryKey = keyof typeof firstLetterData;
...
type PlayQuestion = FirstLetterQuestion & {
  shuffledOptions: string[];
};

const LOCAL_STORAGE_KEY = 'hoc-cung-be-first-letter-scores';
...
function buildPlayQuestions(source: FirstLetterQuestion[], count: number): PlayQuestion[] {
...
function buildQuestionSpeech(question: FirstLetterQuestion) {
  return `${question.prompt}. Từ cần tìm là ${question.word}. Bé hãy chọn chữ cái đầu tiên.`;
}

function buildCorrectSpeech(letter: string, word: string) {
  return `Giỏi lắm. Chữ cái đầu của từ ${word} là ${letter}.`;
}

function buildWrongSpeech(letter: string, word: string) {
  return `Chưa đúng nhé. Chữ cái đầu của từ ${word} là ${letter}.`;
}
...
export default function FirstLetterGame() {
  ...
  useEffect(() => {
    ...
    const categoryInfo = firstLetterData[selectedCategory];
  ...
  const startCategoryGame = (key: CategoryKey) => {
    const sourceQuestions = firstLetterData[key].questions;
  ...
  const handleChoose = (option: string) => {
    ...
    if (option === currentQuestion.correct) {
      ...
      playCorrectSound();
      speakText(buildCorrectSpeech(currentQuestion.correct, currentQuestion.word), {
        lang: 'vi-VN',
        rate: 0.92,
        pitch: 1.12,
      });
      ...
      playWrongSound();
      speakText(buildWrongSpeech(currentQuestion.correct, currentQuestion.word), {
        lang: 'vi-VN',
        rate: 0.9,
        pitch: 1.02,
      });
  ...
  const handleNext = () => {
    ...
    if (currentIndex === questions.length - 1) {
      ...
      if (selectedCategory) {
        speakText(
          buildFinishSpeech(
            firstLetterData[selectedCategory].label,
            score,
            questions.length
          ),
          {
      ...
      <section className="mx-auto max-w-7xl px-6 py-8 lg:px-8 lg:py-12">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé nhìn hình, đọc từ và chọn đúng chữ cái đầu tiên. Trò chơi giúp bé làm quen
            mặt chữ và tăng phản xạ ngôn ngữ.
          </p>
            ...
            {firstLetterCategories.map((category) => (
              ...
              >
                ...
                <p className="mt-2 text-sm leading-7 text-slate-600">
                  {Math.min(QUESTIONS_PER_GAME, category.total)} câu mỗi lượt chơi, vui và dễ nhớ.
                </p>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nhận biết chữ cái đầu tiên',
                  'Làm quen mặt chữ và từ vựng',
                  'Quan sát hình ảnh và liên tưởng',
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé nhìn hình, nhớ tên từ và chọn đúng chữ cái đầu tiên.
          </p>
          ...
          <div className="mt-8 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 text-center ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">Hình minh họa</p>
                <div className="mt-4 text-8xl">{currentQuestion.image}</div>
                <p className="mt-4 text-lg font-bold text-slate-700">{currentQuestion.word}</p>
              </div>
              ...
              {showResult && (
                ...
                >
                  {selected === currentQuestion.correct
                    ? `Chính xác rồi. Chữ cái đầu của từ ${currentQuestion.word} là ${currentQuestion.correct}.`
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {firstLetterData[selectedCategory].icon}
              </div>
              <div>
