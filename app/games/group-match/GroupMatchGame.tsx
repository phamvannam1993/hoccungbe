...
import {
  ...
  groupMatchData,
  type GroupMatchQuestion,
} from './data';
import { speakText, stopSpeaking } from '../../components/edu/utils/speech';

type CategoryKey = keyof typeof groupMatchData;
...
type PlayQuestion = GroupMatchQuestion & {
  shuffledOptions: {
    emoji: string;
    label: string;
    group: string;
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-group-match-scores';
...
function buildPlayQuestions(source: GroupMatchQuestion[], count: number): PlayQuestion[] {
...
function buildQuestionSpeech(question: GroupMatchQuestion) {
  return `${question.prompt}. Bé hãy chọn đủ ${question.requiredCount} hình thuộc nhóm ${question.groupLabel}.`;
}
...
function buildWrongSpeech(groupLabel: string) {
  return `Chưa đúng nhé. Bé cần chọn đủ các hình thuộc nhóm ${groupLabel}.`;
}
...
export default function GroupMatchGame() {
  ...
  useEffect(() => {
    ...
    const categoryInfo = groupMatchData[selectedCategory];
  ...
  const startCategoryGame = (key: CategoryKey) => {
    const sourceQuestions = groupMatchData[key].questions;
  ...
  const toggleSelect = (emoji: string) => {
    ...
    setSelectedItems((prev) => {
      ...
      if (prev.length >= currentQuestion.requiredCount) {
        return prev;
      }
  ...
  const handleCheckAnswer = () => {
    ...
    if (selectedItems.length !== currentQuestion.requiredCount) return;

    stopSpeaking();
    setShowResult(true);

    const selectedOptions = currentQuestion.shuffledOptions.filter((item) =>
      selectedItems.includes(item.emoji)
    );

    const isCorrect = selectedOptions.every(
      (item) => item.group === currentQuestion.correctGroup
    );

    if (isCorrect) {
      setScore((prev) => prev + 1);
      playCorrectSound();
      speakText(buildCorrectSpeech(currentQuestion.groupLabel), {
        lang: 'vi-VN',
        rate: 0.92,
        pitch: 1.12,
      });
      ...
      playWrongSound();
      speakText(buildWrongSpeech(currentQuestion.groupLabel), {
        lang: 'vi-VN',
        rate: 0.9,
        pitch: 1.02,
      });
  ...
  const handleNext = () => {
    ...
    if (currentIndex === questions.length - 1) {
      ...
      if (selectedCategory) {
        speakText(
          buildFinishSpeech(
            groupMatchData[selectedCategory].label,
            score,
            questions.length
          ),
          {
  ...
  const getIsCorrectOption = (emoji: string) => {
    if (!currentQuestion) return false;
    const option = currentQuestion.shuffledOptions.find((item) => item.emoji === emoji);
    return option?.group === currentQuestion.correctGroup;
  };

  const isAnswerCorrect = currentQuestion
    ? currentQuestion.shuffledOptions
        .filter((item) => selectedItems.includes(item.emoji))
        .every((item) => item.group === currentQuestion.correctGroup) &&
      selectedItems.length === currentQuestion.requiredCount
    : false;
      ...
      <section className="mx-auto max-w-7xl px-6 py-8 lg:px-8 lg:py-12">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé nhìn nhiều hình khác nhau và chọn đủ các hình thuộc cùng một nhóm. Trò chơi giúp tăng khả năng phân loại, quan sát và tư duy logic.
          </p>

          <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {groupMatchCategories.map((category) => (
              <button
                key={category.key}
              ...
              >
                ...
                <p className="mt-2 text-sm leading-7 text-slate-600">
                  {Math.min(QUESTIONS_PER_GAME, category.total)} câu mỗi lượt chơi, trực quan và dễ hiểu.
                </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy chọn đủ các hình thuộc đúng nhóm được yêu cầu.
          </p>
          ...
          <div className="mt-8 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 text-center ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">Nhóm cần tìm</p>
                <div className="mt-4 flex items-center justify-center gap-3">
                  <div className="text-5xl">{currentQuestion.groupEmoji}</div>
                  <div className="text-xl font-black text-slate-900">
                    {currentQuestion.groupLabel}
                  </div>
                </div>
                <p className="mt-3 text-sm font-semibold text-slate-600">
                  Đã chọn: {selectedItems.length}/{currentQuestion.requiredCount}
                </p>
              </div>

              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                {currentQuestion.shuffledOptions.map((option) => {
                  const isSelected = selectedItems.includes(option.emoji);
                  const isCorrect = getIsCorrectOption(option.emoji);

                  let buttonClass =
                    'bg-white text-slate-700 ring-slate-200 hover:bg-slate-50';

                  if (isSelected && !showResult) {
                    buttonClass = 'bg-sky-50 text-sky-700 ring-sky-200';
                  }
                  ...
                  return (
                    <button
                      key={`${currentQuestion.id}-${option.emoji}`}
                      onClick={() => toggleSelect(option.emoji)}
                      disabled={showResult}
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <button
                  ...
                  disabled={
                    showResult || selectedItems.length !== currentQuestion.requiredCount
                  }
                  className="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-emerald-500 to-sky-500 px-5 py-3 text-sm font-bold text-white shadow-lg transition duration-300 hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Xác nhận
                </button>
                ...
                <button
                  ...
                  disabled={showResult || selectedItems.length === 0}
                  className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 shadow-sm transition duration-300 hover:-translate-y-0.5 hover:border-slate-300 hover:bg-slate-50 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Bỏ chọn
                </button>
              </div>
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {groupMatchData[selectedCategory].icon}
              </div>
              <div>
