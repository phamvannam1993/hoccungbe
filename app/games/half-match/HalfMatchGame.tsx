...
import {
  halfMatchCategories,
  halfMatchData,
  type HalfMatchCategory,
  type HalfMatchLevel,
  type HalfMatchQuestion,
} from './data';

type CategoryKey = keyof typeof halfMatchData;
...
type PlayQuestion = HalfMatchQuestion & {
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-half-match-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-half-match-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-half-match-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-half-match-stickers';
...
const stickers: StickerItem[] = [
  ...
  },
  {
    id: 'perfect',
    emoji: '👑',
    title: 'Mắt nhìn rất giỏi',
    description: 'Đạt độ chính xác rất cao.',
  },
  {
    id: 'combo-3',
    emoji: '🔥',
    title: 'Combo ghép hình',
...
export default function HalfMatchGame() {
  const [selectedCategory, setSelectedCategory] = useState<CategoryKey | null>(null);
  const [selectedLevel, setSelectedLevel] = useState<HalfMatchLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = halfMatchData[selectedCategory];
  ...
  const startCategoryGame = (key: CategoryKey, level: HalfMatchLevel) => {
    const sourceQuestions = halfMatchData[key].questions;
  ...
  const handleChoose = async (value: string) => {
    ...
    if (passed) {
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(`Chính xác rồi. Đây là ${currentQuestion.fullLabel}`);
      }, 220);
    } else {
      setCombo(0);
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(`Chưa đúng nhé. Đáp án đúng là ${currentQuestion.fullLabel}`);
      }, 220);
    }
  };
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé nhìn nửa hình mẫu rồi chọn đúng phần còn lại để tạo thành hình hoàn chỉnh.
                Trò chơi giúp tăng khả năng quan sát, nhận diện tổng thể và ghép nối logic.
              </p>
            </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {halfMatchCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as HalfMatchLevel[]).map((level) => (
                      <button
                        key={level}
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nhận diện hình tổng thể',
                  'Quan sát chi tiết',
                  'Ghép nối đúng phần còn thiếu',
                  'Tăng tư duy trực quan',
                ].map((item) => (
                  <div
                    key={item}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng quan sát và ghép nối của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {halfMatchData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy nhìn nửa hình và chọn phần ghép đúng để tạo thành hình hoàn chỉnh.
          </p>
          ...
          <div className="mt-6 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 text-center ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">Nửa hình mẫu</p>
                ...
                <div className="mt-5 flex items-center justify-center gap-4">
                  <div className="flex h-24 w-24 items-center justify-center rounded-[28px] bg-white text-5xl shadow-sm ring-1 ring-slate-200">
                    {currentQuestion.leftHalf}
                  </div>

                  <div className="text-4xl font-black text-violet-500">+</div>

                  <div className="flex h-24 w-24 items-center justify-center rounded-[28px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-5xl text-white shadow-sm">
                    ?
                  </div>
                </div>
              </div>
              ...
              {showResult && (
                ...
                >
                  {selected === currentQuestion.correct
                    ? `Chính xác rồi. Đây là ${currentQuestion.fullLabel}.`
                    : `Chưa đúng nhé. Đáp án đúng là ${currentQuestion.fullLabel}.`}
                </div>
              )}
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {halfMatchData[selectedCategory].icon}
              </div>
              <div>
