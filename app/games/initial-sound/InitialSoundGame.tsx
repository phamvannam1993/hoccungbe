...
import {
  initialSoundCategories,
  initialSoundData,
  type InitialSoundCategory,
  type InitialSoundLevel,
  type InitialSoundQuestion,
} from './data';

type CategoryKey = keyof typeof initialSoundData;
...
type PlayQuestion = InitialSoundQuestion & {
  shuffledItems: InitialSoundQuestion['items'];
};
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-initial-sound-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-initial-sound-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-initial-sound-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-initial-sound-stickers';
...
const stickers: StickerItem[] = [
  ...
  },
  {
    ...
    emoji: '🅱️',
    title: 'Bạn của âm bờ',
    description: 'Hoàn thành chủ đề âm bờ.',
  },
  {
    id: 'm-master',
    emoji: 'Ⓜ️',
    title: 'Bạn của âm mờ',
    description: 'Hoàn thành chủ đề âm mờ.',
  },
];
...
export default function InitialSoundGame() {
  const [selectedCategory, setSelectedCategory] = useState<CategoryKey | null>(null);
  const [selectedLevel, setSelectedLevel] = useState<InitialSoundLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = initialSoundData[selectedCategory];
    ...
    if (selectedCategory === 'b') unlockSticker('b-master');
    if (selectedCategory === 'm') unlockSticker('m-master');
  ...
  const speakQuestion = async () => {
    ...
    if (speechEnabled) speakText(
      `${currentQuestion.prompt}. Âm đầu là ${currentQuestion.sound}. ${currentQuestion.hint}`
    );
  };

  const speakSoundOnly = async () => {
    if (!currentQuestion) return;
    await playPromptSound();
    if (speechEnabled) speakText(`Âm đầu là ${currentQuestion.sound}`);
  };
  ...
  const startCategoryGame = (key: CategoryKey, level: InitialSoundLevel) => {
    const sourceQuestions = initialSoundData[key].questions;
  ...
  const handleChoose = async (itemId: string) => {
    ...
    const pickedItem = currentQuestion.items.find((item) => item.id === itemId);
    const passed = pickedItem?.label === currentQuestion.correct;
    ...
    if (passed) {
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(
          `Chính xác rồi. ${currentQuestion.correct} bắt đầu bằng âm ${currentQuestion.sound}`
        );
      }, 220);
    } else {
      setCombo(0);
  ...
  const correctItem = currentQuestion?.items.find(
    (item) => item.label === currentQuestion.correct
  );
    ...
    return (
      <section className="mx-auto max-w-7xl px-6 py-8 lg:px-8 lg:py-12">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">
                Nghe âm đầu chọn hình
              </h1>

              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé nghe âm đầu rồi chọn hình có từ bắt đầu bằng âm đó. Trò chơi giúp rèn
                nhận biết âm đầu, phát triển ngôn ngữ và chuẩn bị cho kỹ năng đọc sớm.
              </p>
            </div>
          ...
          <div className="mt-8">
            <h2 className="text-xl font-black text-slate-900">Chọn âm đầu và mức độ</h2>

            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {initialSoundCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-600">
                    {category.total} câu hỏi theo âm đầu này.
                  </p>

                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as InitialSoundLevel[]).map((level) => (
                      <button
                        key={level}
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nhận biết âm đầu',
                  'Liên kết âm với hình',
                  'Chuẩn bị cho đọc sớm',
                  'Tăng chú ý khi nghe',
                ].map((item) => (
                  <div
                    key={item}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng nghe âm đầu và nhận biết từ của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {initialSoundData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy nghe âm đầu rồi chọn đúng hình.
          </p>
          ...
          <div className="mt-6 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 text-center ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">Âm đầu cần nghe</p>
                <div className="mt-4 text-5xl font-black text-slate-900">
                  {currentQuestion.sound}
                </div>
                <p className="mt-3 text-sm text-slate-500">
                  Bé hãy bấm nghe rồi chọn hình đúng nhé
                </p>
              </div>

              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {currentQuestion.shuffledItems.map((item) => {
                  const isCorrect = item.label === currentQuestion.correct;
              ...
              {showResult && (
                <div
                  className={`mt-6 rounded-2xl px-4 py-4 text-sm font-semibold ${
                    selected &&
                    currentQuestion.shuffledItems.find((x) => x.id === selected)?.label ===
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {initialSoundData[selectedCategory].icon}
              </div>
              <div>
