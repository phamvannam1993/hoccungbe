...
import {
  listenDoCategories,
  listenDoData,
  type ListenDoCategory,
  type ListenDoLevel,
  type ListenDoQuestion,
} from './data';

type CategoryKey = keyof typeof listenDoData;
...
type PlayQuestion = ListenDoQuestion & {
  shuffledItems: ListenDoQuestion['items'];
};
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-listen-do-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-listen-do-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-listen-do-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-listen-do-stickers';
...
export default function ListenAndDoGame() {
  const [selectedCategory, setSelectedCategory] = useState<CategoryKey | null>(null);
  const [selectedLevel, setSelectedLevel] = useState<ListenDoLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = listenDoData[selectedCategory];
  ...
  const startCategoryGame = (key: CategoryKey, level: ListenDoLevel) => {
    const sourceQuestions = listenDoData[key].questions;
  ...
  const handleChoose = async (itemId: string) => {
    ...
    const passed = pickedItem?.emoji === currentQuestion.correct;
    ...
    if (passed) {
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(`Chính xác rồi. ${pickedItem?.label ?? 'Bạn nhỏ đã chọn đúng'}`);
      }, 220);
    } else {
      setCombo(0);
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(`Chưa đúng nhé. Đáp án đúng là ${correctItem?.label ?? 'hình đúng'}`);
      }, 220);
    }
  };
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé nghe yêu cầu ngắn rồi chọn đúng hình phù hợp. Trò chơi giúp bé tăng
                khả năng nghe hiểu, chú ý và phản xạ khi làm theo hướng dẫn.
              </p>
            </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {listenDoCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as ListenDoLevel[]).map((level) => (
                      <button
                        key={level}
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nghe hiểu chỉ dẫn ngắn',
                  'Chọn đúng theo yêu cầu',
                  'Tăng chú ý và phản xạ',
                  'Nhận biết hình, màu, con vật, đồ vật',
                ].map((item) => (
                  <div
                    key={item}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng nghe hiểu và làm theo chỉ dẫn của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {listenDoData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy nghe yêu cầu rồi chọn đúng hình.
          </p>
          ...
          <div className="mt-6 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-semibold text-slate-500">Yêu cầu hiện tại</p>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {currentQuestion.shuffledItems.map((item) => {
                  const isCorrect = item.emoji === currentQuestion.correct;
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {listenDoData[selectedCategory].icon}
              </div>
              <div>
