...
import {
  numberQuantityCategories,
  numberQuantityData,
  type NumberQuantityCategory,
  type NumberQuantityLevel,
  type NumberQuantityQuestion,
} from './data';

type CategoryKey = keyof typeof numberQuantityData;
...
type PlayQuestion = NumberQuantityQuestion & {
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-number-quantity-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-number-quantity-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-number-quantity-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-number-quantity-stickers';
...
const stickers: StickerItem[] = [
  ...
  },
  {
    id: 'fruits-master',
    emoji: '🍎',
    title: 'Bạn của trái cây',
    description: 'Hoàn thành chủ đề trái cây.',
  },
  {
    id: 'school-master',
    emoji: '✏️',
    title: 'Bạn của lớp học',
    description: 'Hoàn thành chủ đề đồ dùng học tập.',
  },
];
...
export default function NumberQuantityMatchGame() {
  const [selectedCategory, setSelectedCategory] = useState<CategoryKey | null>(null);
  const [selectedLevel, setSelectedLevel] = useState<NumberQuantityLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = numberQuantityData[selectedCategory];
    ...
    if (selectedCategory === 'fruits') unlockSticker('fruits-master');
    if (selectedCategory === 'school') unlockSticker('school-master');
  ...
  const speakPrompt = async () => {
    ...
    if (speechEnabled) speakText(
      `${currentQuestion.prompt}. ${currentQuestion.hint}. Bé hãy đếm rồi chọn số đúng`
    );
  };

  const speakQuantity = async () => {
    if (!currentQuestion) return;
    await playPromptSound();
  ...
  const startCategoryGame = (key: CategoryKey, level: NumberQuantityLevel) => {
    const sourceQuestions = numberQuantityData[key].questions;
  ...
  const handleChoose = async (value: number) => {
    ...
    const passed = value === currentQuestion.correctNumber;
    ...
    if (passed) {
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(`Chính xác rồi. Có ${currentQuestion.correctNumber} hình`);
      }, 220);
    } else {
      setCombo(0);
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(`Chưa đúng nhé. Đáp án đúng là số ${currentQuestion.correctNumber}`);
      }, 220);
    }
  };
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé quan sát số lượng hình rồi chọn đúng chữ số tương ứng. Trò chơi giúp
                bé luyện đếm và kết nối số với hình ảnh trực quan.
              </p>
            </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {numberQuantityCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as NumberQuantityLevel[]).map((level) => (
                      <button
                        key={level}
            ...
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nhận biết chữ số',
                  'Đếm đúng số lượng',
                  'Liên kết số với hình',
                  'Tăng phản xạ toán học cơ bản',
                ].map((item) => (
                  <div
                    key={item}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng đếm số và nhận biết số lượng của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {numberQuantityData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy nhìn số lượng hình rồi chọn đúng con số tương ứng.
          </p>
          ...
          <div className="mt-6 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 text-center ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">Số lượng cần đếm</p>
                <div className="mt-4 text-4xl leading-relaxed">{currentQuestion.quantityEmoji}</div>
                <p className="mt-3 text-sm text-slate-500">{currentQuestion.quantityLabel}</p>
              </div>

              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {currentQuestion.shuffledOptions.map((value) => {
                  const isCorrect = value === currentQuestion.correctNumber;
              ...
              {showResult && (
                <div
                  className={`mt-6 rounded-2xl px-4 py-4 text-sm font-semibold ${
                    selected === currentQuestion.correctNumber
                      ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100'
                      : 'bg-rose-50 text-rose-700 ring-1 ring-rose-100'
                  }`}
                >
                  {selected === currentQuestion.correctNumber
                    ? `Chính xác rồi. Có ${currentQuestion.correctNumber} hình.`
                    : `Chưa đúng nhé. Đáp án đúng là số ${currentQuestion.correctNumber}.`}
                </div>
              )}
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {numberQuantityData[selectedCategory].icon}
              </div>
              <div>
