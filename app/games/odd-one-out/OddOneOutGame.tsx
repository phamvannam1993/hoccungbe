...
type CategoryKey = keyof typeof oddOneOutData;
...
type PlayQuestion = OddOneOutQuestion & {
  shuffledItems: OddOneOutQuestion['items'];
};
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-odd-one-out-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-odd-one-out-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-odd-one-out-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-odd-one-out-stickers';
...
export default function OddOneOutGame() {
  ...
  const [selectedLevel, setSelectedLevel] = useState<OddOneOutLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = oddOneOutData[selectedCategory];
  ...
  const startCategoryGame = (key: CategoryKey, level: OddOneOutLevel) => {
    const sourceQuestions = oddOneOutData[key].questions;
  ...
  const handleChoose = async (itemId: string) => {
    ...
    if (passed) {
      ...
      setTimeout(() => {
        if (speechEnabled) speakText('Chính xác rồi. Bạn nhỏ đã tìm đúng vật khác nhóm');
      }, 220);
    } else {
      setCombo(0);
      ...
      setTimeout(() => {
        if (speechEnabled) speakText('Chưa đúng nhé. Bé thử lại câu tiếp theo nào');
      }, 220);
    }
  };
  ...
  const correctEmoji = currentQuestion?.correct;
  const correctItem = currentQuestion?.items.find((item) => item.emoji === correctEmoji);
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé quan sát các hình rồi chọn ra vật không cùng nhóm với những hình còn lại.
                Trò chơi giúp rèn khả năng phân loại, so sánh và suy luận logic.
              </p>
            </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {oddOneOutCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as OddOneOutLevel[]).map((level) => (
                      <button
                        key={level}
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nhận biết điểm chung và khác biệt',
                  'Phân loại theo nhóm',
                  'Tư duy logic cơ bản',
                  'Quan sát nhanh và chính xác',
                ].map((item) => (
                  <div
                    key={item}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng phân loại và quan sát của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {oddOneOutData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy nhìn các hình và chọn ra vật khác nhóm.
          </p>
            ...
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              {showResult && (
                ...
                >
                  ...
                  currentQuestion.shuffledItems.find((x) => x.id === selected)?.emoji ===
                    currentQuestion.correct
                    ? 'Chính xác rồi. Bạn nhỏ đã tìm đúng vật khác nhóm.'
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {oddOneOutData[selectedCategory].icon}
              </div>
              <div>
