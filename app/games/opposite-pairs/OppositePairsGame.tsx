...
import {
  oppositeCategories,
  oppositeData,
  type OppositeCategory,
  type OppositeLevel,
  type OppositeQuestion,
} from './data';

type CategoryKey = keyof typeof oppositeData;
...
type PlayQuestion = OppositeQuestion & {
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-opposite-pairs-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-opposite-pairs-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-opposite-pairs-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-opposite-pairs-stickers';
...
const stickers: StickerItem[] = [
  ...
  },
  {
    id: 'size-master',
    emoji: '📏',
    title: 'Bé hiểu so sánh',
    description: 'Hoàn thành chủ đề kích thước.',
  },
  {
    id: 'emotion-master',
    emoji: '💖',
    title: 'Bé hiểu cảm xúc',
    description: 'Hoàn thành chủ đề cảm xúc.',
  },
];
...
export default function OppositePairsGame() {
  ...
  const [selectedLevel, setSelectedLevel] = useState<OppositeLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = oppositeData[selectedCategory];
    ...
    if (selectedCategory === 'size') unlockSticker('size-master');
    if (selectedCategory === 'emotion') unlockSticker('emotion-master');
  ...
  const startCategoryGame = (key: CategoryKey, level: OppositeLevel) => {
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé đọc từ gợi ý rồi chọn từ có nghĩa trái ngược. Trò chơi giúp tăng vốn từ,
                khả năng so sánh và tư duy ngôn ngữ một cách tự nhiên.
              </p>
            </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {oppositeCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as OppositeLevel[]).map((level) => (
                      <button
                        key={level}
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Hiểu các khái niệm trái ngược',
                  'Tăng vốn từ tiếng Việt',
                  'Phản xạ ngôn ngữ tốt hơn',
                  'Nhận biết cảm xúc và so sánh tốt hơn',
                ].map((item) => (
                  <div
                    key={item}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng ngôn ngữ và so sánh của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {oppositeData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé đọc từ gợi ý rồi chọn từ có nghĩa trái ngược.
          </p>
          ...
          <div className="mt-6 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 text-center ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">Từ gợi ý</p>

                <div className="mt-4 text-6xl">{currentQuestion.leftEmoji}</div>
                ...
                {currentQuestion.shuffledOptions.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={showResult}
                      className={`rounded-[28px] px-4 py-5 text-center text-2xl font-black shadow-sm ring-1 transition duration-300 ${buttonClass}`}
                    >
                      {option}
                    </button>
                  );
              ...
              {showResult && (
                ...
                >
                  {selected === currentQuestion.correct
                    ? `Chính xác rồi. Từ trái nghĩa là ${currentQuestion.correct}.`
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {oppositeData[selectedCategory].icon}
              </div>
              <div>
