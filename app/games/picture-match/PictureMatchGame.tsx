...
import styles from "./PictureMatchGame.module.css";
...
export default function PictureMatchGame() {
  ...
  const [matched, setMatched] = useState<Set<string>>(new Set()); // pairIds matched
  ...
  const handleRight = (item: MatchItem) => {
    ...
    if (leftItem.pairId === item.pairId) {
      ...
      window.setTimeout(() => {
        setWrongPair(null);
        setSelectedLeft(null);
        // Count mistakes — 2 mistakes → game over? Or keep playing
        // For this game: wrong = keep trying (no game over on mismatch)
      }, 700);
    }
  };
      ...
      <section className={styles.game}>
          ...
          <div className={styles.overlay}>
            <div className={styles.overlayBox}>
              ...
              <ul className={styles.instructions}>
                ...
                <li>📈 Độ khó tăng dần theo vòng chơi.</li>
              </ul>
              <button className={styles.startButton} onClick={handleStart}>▶ Bắt đầu chơi</button>
            </div>
          </div>
        )}
        ...
        {gameOver && (
          <div className={styles.overlay}>
            <div className={styles.overlayBox}>
              ...
              <button className={styles.startButton} onClick={handleRestart}>↻ Chơi lại</button>
            </div>
          </div>
        )}
        ...
        <header className={styles.header}>
          <div>
            ...
            <span className={styles.subtitle}>Vòng {round + 1} · ⭐ {bestRound}</span>
          </div>
          <button className={styles.audioBtn} onClick={() => speakText(level.instruction, { lang: "vi-VN" })}>🔊</button>
        </header>
          ...
          <div className={styles.columns}>
            ...
            <div className={styles.connectorArea}>
              <svg className={styles.svg} viewBox="0 0 60 100" preserveAspectRatio="none">
                {/* Visual hint lines for matched pairs */}
              </svg>
