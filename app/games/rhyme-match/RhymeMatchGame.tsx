...
import {
  rhymeMatchCategories,
  rhymeMatchData,
  type RhymeMatchCategory,
  type RhymeMatchLevel,
  type RhymeMatchQuestion,
} from './data';

type CategoryKey = keyof typeof rhymeMatchData;
...
type PlayQuestion = RhymeMatchQuestion & {
  shuffledOptions: RhymeMatchQuestion['options'];
};
...
const LOCAL_STORAGE_KEY = 'hoc-cung-be-rhyme-match-scores';
const SOUND_ENABLED_KEY = 'hoc-cung-be-rhyme-match-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-rhyme-match-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-rhyme-match-stickers';
...
const stickers: StickerItem[] = [
  ...
  },
  {
    id: 'perfect',
    emoji: '👑',
    title: 'Nghe vần rất giỏi',
    description: 'Đạt độ chính xác rất cao.',
  },
  {
    id: 'combo-3',
    emoji: '🔥',
    title: 'Combo vần điệu',
    description: 'Đạt 3 câu đúng liên tiếp.',
  },
  {
    id: 'an-master',
    emoji: '🌟',
    title: 'Bạn của vần an',
    description: 'Hoàn thành chủ đề vần an.',
  },
  {
    id: 'mixed-master',
    emoji: '✨',
    title: 'Bé nghe vần siêu tốt',
...
export default function RhymeMatchGame() {
  const [selectedCategory, setSelectedCategory] = useState<CategoryKey | null>(null);
  const [selectedLevel, setSelectedLevel] = useState<RhymeMatchLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = rhymeMatchData[selectedCategory];
    ...
    if (selectedCategory === 'an') unlockSticker('an-master');
  ...
  const speakPrompt = async () => {
    ...
    if (speechEnabled) speakText(
      `${currentQuestion.prompt}. Từ mẫu là ${currentQuestion.targetWord}. ${currentQuestion.hint}`
    );
  };
  ...
  const speakTarget = async () => {
    if (!currentQuestion) return;
    await playPromptSound();
    if (speechEnabled) speakText(`Từ mẫu là ${currentQuestion.targetWord}`);
  };
  ...
  const startCategoryGame = (key: CategoryKey, level: RhymeMatchLevel) => {
    const sourceQuestions = rhymeMatchData[key].questions;
  ...
  const handleChoose = async (value: string) => {
    ...
    if (passed) {
      ...
      setTimeout(() => {
        if (speechEnabled) speakText(
          `Chính xác rồi. ${currentQuestion.correct} có vần giống với ${currentQuestion.targetWord}`
        );
      }, 220);
    } else {
      setCombo(0);
    ...
    return (
      <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé nhìn và nghe từ mẫu, sau đó chọn từ có vần giống nhau. Trò chơi giúp
                bé luyện nhận biết vần, nghe âm cuối và hỗ trợ chuẩn bị cho đọc sớm.
              </p>
            </div>
          ...
          <div className="mt-8">
            <h2 className="text-xl font-black text-slate-900">Chọn nhóm vần và mức độ</h2>

            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {rhymeMatchCategories.map((category) => (
                <div
                  key={category.key}
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-600">
                    {category.total} câu hỏi theo nhóm vần này.
                  </p>

                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as RhymeMatchLevel[]).map((level) => (
                      <button
                        key={level}
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nhận biết vần giống nhau',
                  'Nghe và so sánh âm cuối',
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng nghe vần và nhận biết âm cuối của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {rhymeMatchData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy nhìn từ mẫu và chọn từ có vần giống nhau.
          </p>
          ...
          <div className="mt-6 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 text-center ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">Từ mẫu</p>

                <div className="mt-5 flex items-center justify-center gap-4">
                  <div className="flex h-24 w-24 items-center justify-center rounded-[28px] bg-white text-5xl shadow-sm ring-1 ring-slate-200">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {currentQuestion.shuffledOptions.map((option) => {
                  const isCorrect = option.word === currentQuestion.correct;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 text-xl font-black">{option.word}</div>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {rhymeMatchData[selectedCategory].icon}
              </div>
              <div>
