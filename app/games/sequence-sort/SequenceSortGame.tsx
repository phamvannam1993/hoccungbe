...
type CategoryKey = keyof typeof sequenceData;
...
type PlayQuestion = SequenceQuestion & {
  shuffledItems: SequenceItem[];
};
...
const SOUND_ENABLED_KEY = 'hoc-cung-be-sequence-sort-sound-enabled';
const SPEECH_ENABLED_KEY = 'hoc-cung-be-sequence-sort-speech-enabled';
const STICKERS_KEY = 'hoc-cung-be-sequence-sort-stickers';
...
const stickers: StickerItem[] = [
  ...
  },
  {
    id: 'quantity-master',
    emoji: '🔢',
    title: 'Bạn của con số',
    description: 'Hoàn thành chủ đề số lượng.',
  },
  {
    id: 'height-master',
    emoji: '🏢',
    title: 'Bé giỏi so sánh',
    description: 'Hoàn thành chủ đề chiều cao.',
  },
];
...
function buildPlayQuestions(
  ...
  return shuffleArray(sourceToUse)
    ...
    .map((question) => ({
      ...question,
      shuffledItems: shuffleArray(question.items),
    }));
}
...
export default function SequenceSortGame() {
  ...
  const [selectedLevel, setSelectedLevel] = useState<SequenceLevel>('easy');
  ...
  useEffect(() => {
    ...
    const categoryInfo = sequenceData[selectedCategory];
    ...
    if (selectedCategory === 'quantity') unlockSticker('quantity-master');
    if (selectedCategory === 'height') unlockSticker('height-master');
  ...
  const startCategoryGame = (key: CategoryKey, level: SequenceLevel) => {
    const sourceQuestions = sequenceData[key].questions;
  ...
  const handlePickItem = async (item: SequenceItem) => {
    if (!currentQuestion || showResult) return;
    if (orderedItems.some((x) => x.id === item.id)) return;
  ...
  const handleUndo = () => {
    ...
    setOrderedItems((prev) => prev.slice(0, -1));
  };
  ...
  const handleCheck = async () => {
    if (!currentQuestion || showResult) return;
    if (orderedItems.length !== currentQuestion.items.length) return;

    const correctIds = [...currentQuestion.items]
      .sort((a, b) =>
        currentQuestion.direction === 'asc' ? a.value - b.value : b.value - a.value
      )
      .map((item) => item.id);

    const selectedIds = orderedItems.map((item) => item.id);
    const passed = JSON.stringify(correctIds) === JSON.stringify(selectedIds);
    ...
    if (passed) {
      ...
      setTimeout(() => {
        speakVietnamese('Chưa đúng nhé. Bé thử lại câu tiếp theo nào');
      }, 220);
    }
  };
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                Bé quan sát các hình rồi sắp xếp đúng theo thứ tự yêu cầu như nhỏ đến lớn,
                ngắn đến dài, ít đến nhiều hoặc thấp đến cao.
              </p>
            </div>
              ...
              {sequenceCategories.map((category) => (
                ...
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {(['easy', 'medium', 'hard'] as SequenceLevel[]).map((level) => (
                      <button
                        key={level}
            ...
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  ...
                  'Nhận biết quy mô và số lượng',
                  'Tăng tư duy logic',
                  'Quan sát và ra quyết định tốt hơn',
                ].map((item) => (
                  <div
                    key={item}
        ...
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-slate-600">
              Kết quả đã được lưu lại để phụ huynh theo dõi khả năng sắp xếp và tư duy của bé.
            </p>
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700 ring-1 ring-violet-100">
                {sequenceData[selectedCategory].label}
              </span>
              <span
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé hãy chọn các hình theo đúng thứ tự được yêu cầu.
          </p>
            ...
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-sky-50 p-6 ring-1 ring-sky-100">
                <p className="text-sm font-semibold text-slate-500">
                  Chạm theo thứ tự đúng
                </p>

                <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {currentQuestion.shuffledItems.map((item) => {
                    const picked = orderedItems.some((x) => x.id === item.id);
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-[24px] px-4 py-5 text-center shadow-sm ring-1 transition duration-300 ${
                          picked
              ...
              <div className="mt-6 rounded-3xl bg-emerald-50 p-6 ring-1 ring-emerald-100">
                ...
                <div className="mt-4 flex min-h-[92px] flex-wrap items-center gap-3">
                  {orderedItems.length === 0 ? (
                    <div className="text-sm text-slate-500">
                      Chưa có lựa chọn nào. Bé hãy bấm theo thứ tự nhé.
                    </div>
                  ) : (
                    orderedItems.map((item, index) => (
                      <div
                        key={`${item.id}-${index}`}
                        className="flex items-center gap-2 rounded-2xl bg-white px-4 py-3 ring-1 ring-emerald-100"
                      >
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <button
                    onClick={handleUndo}
                    disabled={orderedItems.length === 0 || showResult}
                    className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    ↩️ Hoàn tác
                  </button>
                  ...
                  <button
                    onClick={handleCheck}
                    disabled={orderedItems.length !== currentQuestion.items.length || showResult}
                    className="rounded-full bg-gradient-to-r from-sky-500 to-violet-500 px-5 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Kiểm tra
                  </button>
                </div>
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {sequenceData[selectedCategory].icon}
              </div>
              <div>
