...
import {
  shadowMatchCategories,
  shadowMatchData,
  type ShadowMatchQuestion,
} from './data';

import { speakText, stopSpeaking } from '../../components/edu/utils/speech';

function buildQuestionSpeech(categoryLabel: string, question: ShadowMatchQuestion) {
    return `Chủ đề ${categoryLabel}. ${question.prompt}. Bé hãy nhìn bóng và chọn đúng đồ vật.`;
}

function buildCorrectSpeech() {
    return 'Giỏi lắm. Bé chọn đúng rồi.';
}

function buildWrongSpeech(correct: string) {
    return `Chưa đúng nhé. Đáp án đúng là ${correct}.`;
}
...
type PlayQuestion = ShadowMatchQuestion & {
  shuffledOptions: string[];
};

const LOCAL_STORAGE_KEY = 'hoc-cung-be-shadow-match-scores';
...
function buildPlayQuestions(source: ShadowMatchQuestion[], count: number): PlayQuestion[] {
...
export default function ShadowMatchGame() {
  ...
  useEffect(() => {
    if (!selectedCategory || !currentQuestion || finished) return;
    ...
    const timer = setTimeout(() => {
      speakText(
        buildQuestionSpeech(
          shadowMatchData[selectedCategory].label,
          currentQuestion
        ),
        {
          lang: 'vi-VN',
  ...
  }, [selectedCategory, currentQuestion, finished]);

  const startCategoryGame = (key: CategoryKey) => {
    const sourceQuestions = shadowMatchData[key].questions;
  ...
  const handleChoose = (option: string) => {
    ...
    if (option === currentQuestion.correct) {
      ...
      playWrongSound();
      speakText(buildWrongSpeech(currentQuestion.correct), {
        lang: 'vi-VN',
        rate: 0.9,
        pitch: 1.02,
      });
      ...
      <section className="mx-auto max-w-7xl px-6 py-8 lg:px-8 lg:py-12">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">
            Ghép bóng với đồ vật
          </h1>

          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé nhìn hình bóng của đồ vật và chọn đúng đáp án tương ứng. Trò chơi giúp
            rèn quan sát, nhận diện hình dạng và phản xạ chọn nhanh.
          </p>

          <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {shadowMatchCategories.map((category) => (
              <button
                key={category.key}
              ...
              >
                ...
                <p className="mt-2 text-sm leading-7 text-slate-600">
                  {Math.min(QUESTIONS_PER_GAME, category.total)} câu mỗi lượt chơi, vui và trực quan.
                </p>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-[30px] bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  'Nhận biết hình dạng đồ vật',
                  'Quan sát nhanh hơn',
                  'Liên kết hình bóng với vật thật',
                  'Tăng tập trung khi chọn đáp án',
                ].map((item) => (
                  <div
                    key={item}
  ...
  if (finished) {
    const categoryInfo = shadowMatchData[selectedCategory];
    ...
    <section className="mx-auto max-w-6xl px-6 py-8 lg:px-8 lg:py-12">
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-[36px] bg-white p-6 shadow-sm ring-1 ring-slate-100 lg:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
            Bé nhìn hình bóng và chọn đúng đồ vật tương ứng. Trò chơi giúp tăng khả năng
            quan sát, liên tưởng và nhận diện hình dạng.
          </p>
          ...
          <div className="mt-8 rounded-[30px] bg-gradient-to-br from-sky-100 via-violet-50 to-pink-100 p-5">
            <div className="rounded-[24px] bg-white p-5 shadow-inner">
              ...
              <div className="mt-6 rounded-3xl bg-slate-900 p-8 text-center ring-1 ring-slate-800">
                <p className="text-sm font-semibold text-white/60">Hình bóng cần tìm</p>
                <div className="mt-5 text-8xl brightness-0 invert">
                  {currentQuestion.shadow}
                </div>
              </div>
          ...
          <div className="rounded-[30px] bg-white p-6 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-5 flex items-center gap-4 rounded-3xl bg-sky-50 p-5 ring-1 ring-sky-100">
              <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-gradient-to-br from-yellow-300 via-pink-400 to-violet-500 text-3xl shadow-lg">
                {shadowMatchData[selectedCategory].icon}
              </div>
              <div>
